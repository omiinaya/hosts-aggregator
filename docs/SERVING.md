...
This document explains how to serve the aggregated hosts file for Pi-hole, AdGuard Home, and other DNS filtering systems.
...
- **Description**: Serves the complete unified hosts file with headers

### Raw Hosts File (No Headers)
- **URL**: `http://your-server:3010/api/serve/hosts/raw`
- **Content-Type**: `text/plain`
- **Description**: Serves only the domain blocking entries (no comments/headers)
...
3. Add a new custom blocklist:
   ```
   http://your-server:3010/api/serve/hosts/raw
   ```
4. Click **Save**
...
1. Create a script to download the hosts file periodically:
   ```bash
   #!/bin/bash
   curl -s http://localhost:3010/api/serve/hosts/raw > /etc/pihole/custom.list
   pihole restartdns
   ```
...
1. Add to AdGuard Home configuration:
   ```yaml
...
2. Create a script to update the file:
   ```bash
   #!/bin/bash
   curl -s http://localhost:3010/api/serve/hosts/raw > /path/to/hosts-aggregator/hosts.txt
...
# Download hosts file
if [ -n "$TOKEN" ]; then
    curl -H "Authorization: Bearer $TOKEN" -s "$SERVER_URL/api/serve/hosts/raw" > /etc/pihole/custom.list
else
    curl -s "$SERVER_URL/api/serve/hosts/raw" > /etc/pihole/custom.list
fi
...
# Download hosts file
if [ -n "$TOKEN" ]; then
    curl -H "Authorization: Bearer $TOKEN" -s "$SERVER_URL/api/serve/hosts/raw" > "$HOSTS_FILE"
else
    curl -s "$SERVER_URL/api/serve/hosts/raw" > "$HOSTS_FILE"
fi
